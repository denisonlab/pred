    ...
    case 2
        ...
        while iTrial<=nTrials
           ...
           if plaidStatus==1 && strcmp('0)',targetResponseKeyName) % response CW
            ...
            if plaidStatus==1 && precueValidity==1
                x=stairIdxExp;
            elseif  plaidStatus==1 && precueValidity==2
                x=stairIdxUn;
            end
            if plaidStatus==1
                WaitSecs(5);
                fprintf('stairID %d, correct %d', x, correct)
                debugCorrect = sprintf('Correct= %d , StairID= %d, Precue= %d, Stand=%d, Grating=%d, Static=%d, DiffMult=%d,',correct,x,precueValidity,sOrientation,gOrientation,staticGrating,differenceMultiplier);
                DrawFormattedText(window, debugCorrect, 'center', cy-imRect(1)/2, [1 1 1]*white);
                Screen('Flip', window, timeBlank1 + p.imDur - slack);
                KbWait(devNum);
            end
