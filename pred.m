...
%HideCursor(window,-1);
 ...
 switch p.task % task and demo
    case 1 % DEMO
        ...
        Screen('DrawTexture', window, tex_plaid{8,2}, [], imRect, 45);
        ...
        for iTrial = 1:nTrials % the iteration in the trial loop
           ...
           %% Store trial information
            ...
            if plaidStatus==1 % if this variable is 1, this trial is not the first grating trial
                if precueValidity==1
                    [stairIdxExp lastFewAccExp]=updateStaircase(p.stairs, stairIdxExp, lastFewAccExp, correct); % get new stair index value
    
                elseif precueValidity==2
                    [stairIdxUn lastFewAccUnexp]=updateStaircase(p.stairs, stairIdxUn, lastFewAccUnexp, correct); % get new stair index value                    
                end
            end
            if plaidStatus==1
                ...
                d.gratingPhase(iTrial) = gratingPhase;
             ...
             elseif plaidStatus==2 %if this is a waffle trial
                ...
                d.differenceMultiplier(iTrial)=NaN; %store difference multiplier
                d.staticGrating(iTrial)=NaN; %store the static grating index
                d.gratingContrast(iTrial) = NaN; %store contrast of test stimuli
                d.gratingSPF(iTrial) = NaN; %store SPF of test stimuli
                d.gratingPhase(iTrial) = NaN;

                d.orientation(iTrial) = NaN; %store orientation of trial
                d.standardPhase(iTrial) = NaN; %store phase of standard stimuli
                d.standardContrast(iTrial) = NaN; % store contrast of standard stimuli
                d.standardSPF(iTrial) = NaN; %store SPF of standard stimuli
            end
            %% Store trial info
            p.trials1=trials1;
            p.trials2=trials2;
    ...
    case 2
        ...
        while iTrial<=nTrials
            ...
            if trialIdx<=size(trials1,1)
                ...
                fprintf('diffMult %d, precue %d, static %d,  orient %d, s phase %d, g phase %d, s SPF %d, g SPF %d \n', differenceMultiplier, ...
                    precueValidity, staticGrating,Orientation,standardPhase,gratingPhase,standardSPF,gratingSPF);
                    ...
                    while GetSecs < timeTone + p.toneSOA - p.eyeSlack && ~stopThisTrial
                        ...
                        fixations = [fixations fixation];
                          ...
                          if fixation==0
                            ...
                            fields={'timeEnd','timeElapsed','timeTargetResponse','targetRT','targetResponseKey',
                            'timeSpentSaving','timeBlank2','timeT','timeS','timeBlank1'};
                            % save data
                            for iF = 1:numel(fields)
                                d.(fields{iF})(iTrial) = NaN; 
                            end
                    ...
                    while GetSecs < timeS + p.imDur - p.eyeSlack && ~stopThisTrial
                        ...
                        fixations = [fixations fixation];
                          ...
                          if fixation==0
                            ...
                            fields={'timeEnd','timeElapsed','timeTargetResponse','targetRT','targetResponseKey',
                            'timeSpentSaving','timeBlank2','timeT','timeBlank1'};
                            % save data
                            for iF = 1:numel(fields)
                                d.(fields{iF})(iTrial) = NaN; 
                            end
                    ...
                    while GetSecs < timeBlank1 + p.standSOA - p.eyeSlack && ~stopThisTrial
                        ...
                        fixations = [fixations fixation];
                          ...
                          if fixation==0
                            ...
                            d.timeFix(iTrial)=timeFix;
                            d.timeS(iTrial)=timeS;
                            d.timeBlank1(iTrial)=timeBlank1;
                            fields={'timeEnd','timeElapsed','timeTargetResponse','targetRT','targetResponseKey',
                            'timeSpentSaving','timeBlank2','timeT'};
                            % save data
                            for iF = 1:numel(fields)
                                d.(fields{iF})(iTrial) = NaN; 
                            end
                    ...
                    while GetSecs < timeT + p.imDur- p.eyeSlack && ~stopThisTrial
                        ...
                        fixations = [fixations fixation];
                          ...
                          if fixation==0
                            ...
                            d.timeT(iTrial)=timeT;
                            fields={'timeEnd','timeElapsed','timeTargetResponse','targetRT','targetResponseKey',
                            'timeBlank1','timeSpentSaving','timeBlank2'};
                            % save data
                            for iF = 1:numel(fields)
                                d.(fields{iF})(iTrial) = NaN; 
                            end
                ...
                if p.eyeTracking
                    while GetSecs < timeTone + p.toneSOA - p.eyeSlack && ~stopThisTrial
                        ...
                        if fixation==0
                            ...
                            d.timeFix(iTrial)=timeFix;
                            fields={'timeEnd','timeElapsed','timeTargetResponse','targetRT','targetResponseKey',
                            'timeS','timeBlank1','timeSpentSaving,timeT'};
                            % save data
                            for iF = 1:numel(fields)
                                d.(fields{iF})(iTrial) = NaN; 
                            end
                ...
                if p.eyeTracking
                    while GetSecs < timeS + p.imDur- p.eyeSlack && ~stopThisTrial
                        ...
                        if fixation==0
                            ...
                            d.timeS(iTrial)=timeS;
                            fields={'timeEnd','timeElapsed','timeTargetResponse','targetRT','targetResponseKey',
                            'timeBlank1','timeSpentSaving,timeT'};
                            % save data
                            for iF = 1:numel(fields)
                                d.(fields{iF})(iTrial) = NaN; 
                            end
           ...
           %%
            ...
            if plaidStatus==1 && p.debug
                ...
                elseif precueValidity==2
                    fprintf('stairID %d, correct %d \n', stairIdxUn, correct)
                    debugCorrect = sprintf('Correct= %d , StairID= %d, Precue= %d, Stand=%d, Grating=%d, Static=%d, DiffMult=%d,',correct,stairIdxUn,precueValidity,sOrientation,gOrientation,staticGrating,differenceMultiplier);                    
                end
                DrawFormattedText(window, debugCorrect, 'center', cy-imRect(1)/2, [1 1 1]*white);
                Screen('Flip', window, timeBug + p.imDur - slack);
                KbWait(devNum);
            end
